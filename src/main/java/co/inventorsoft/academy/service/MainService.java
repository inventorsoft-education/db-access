...
public class MainService {
    ...
    public void start() {
        ...
        System.out.println("*****" + GREEN.getValue() + "Round" + RESET.getValue()
                ...
                + RESET.getValue() + "***************" + GREEN.getValue() + "Team 2"
                + RESET.getValue() + "**********************************" + GREEN.getValue()
                + "Score" + RESET.getValue() + "****");
        while (teams.size() != 1) {
            Collections.shuffle(teams);
            String roundName = teams.size() == 2 ? "Final" : "1/" + teams.size() / 2;
    ...
    private void calculateResult(List<Team> teams, String name, String round) {
        ...
        for (int i = 0; i < size; i += 2) {
            ...
            System.out.println(String.format("*" + BLUE.getValue() + "%8s\t" + RESET.getValue()
                            + "   * " + YELLOW.getValue() + "%40s - %-40s" + RESET.getValue()
                            + " * " + CYAN.getValue() + "%5s:%-5s" + RESET.getValue() + " *%n",
                            match.getRound(), team1.getName(), team2.getName(),
                            match.getPointsTeam1(), match.getPointsTeam2()));
    ...
    public void winner() {
        ...
        System.out.println("***********************************************************"
                + "*********************************************************");
        System.out.println(GREEN.getValue() + "\t\t\tThe Winner of tournament "
                + result.get(0) + " is " + result.get(1) + " in " + result.get(2));
        System.out.println(RESET.getValue() + "********************************************"
                + "************************************************************************");
    }
}